...
app.get('/export_feedback_full', async (req, res) => {
  const workbook = new Excel.Workbook();
  const sheet = workbook.addWorksheet('Sheet1');
  sheet.addRow(["STT", "ID", "DRIVER", "STAR", "CONTENT", "EXPERIENCE", "STATUS", "DATETIME1", "CUSTOMER NUMBER","CUSTOMER NAME","FROM","TO","DATETIME2"]);
  try {
    const data = await feedbackModel.find();
    if (!data || data.length === 0) {
      return res.status(404).json({
        status: false,
        message: 'No feedback2 found',
        totalResult: 0,
        data: null,
      });
    }
    const responseData = [];
    await Promise.all(data.map(async (feedbackItem, index) => {
      const trip = feedbackItem.trip;
      const tripdata = trip
        ? await tripModel.findById(trip)
        : null;
      responseData[index] = {
        ...feedbackItem._doc,
        trip: tripdata || null,
      };
      // console.log(`Trip for feedback item ${index + 1}:`, tripdata);
    }));
    responseData.forEach((item, index) => {
      const customerNumber = item.trip && item.trip.customer_number ? item.trip.customer_number : "...";
      const customerName = item.trip && item.trip.customer_name ? item.trip.customer_name : "...";
      const from = item.trip && item.trip.from ? item.trip.from : "...";
      const to = item.trip && item.trip.to ? item.trip.to : "...";
      const datetime = item.trip && item.trip.createdAt ? item.trip.createdAt : "...";
      
      sheet.addRow([index + 1, item.id, item.driver, item.star, item.content,(item.experience).toString() , item.status, (item.createdAt).toString(), customerNumber,customerName,from,to,datetime.toString()]);
    });
    const formattedTimestamp = getFormattedTimestamp();
    const randomString = generateRandomString(3); // Generate a random string with length 5
    const excelFileName = `feedback_history_${formattedTimestamp}_${randomString}.xlsx`; // Generate a unique file name
